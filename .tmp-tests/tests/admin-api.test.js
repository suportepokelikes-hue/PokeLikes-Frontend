...
(0, node_test_1.default)('admin catalog api functions keep catalog and affiliate settings aligned with the validated contract', async () => {
    ...
    try {
        ...
        const settingsList = await (0, admin_1.listAdminCatalogAffiliateSettings)('token-123', { search: 'instagram', page: 3, pageSize: 25 });
        const updatedSettings = await (0, admin_1.updateAdminCatalogAffiliateSettings)('token-123', 'svc-1', {
            isAffiliateEnabled: true,
            affiliateCommissionPercent: '12.5',
        });
        strict_1.default.deepEqual(settingsList.items[0], {
            catalogServiceId: 'svc-1',
            affiliateEnabled: true,
            affiliateCommissionPercent: '12.5',
            catalogService: { id: 'svc-1', name: 'Instagram Likes' },
        });
        strict_1.default.deepEqual(updatedSettings, {
            catalogServiceId: 'svc-1',
            affiliateEnabled: true,
            affiliateCommissionPercent: '12.5',
            catalogService: { id: 'svc-1', name: 'Instagram Likes' },
...
(0, node_test_1.default)('admin affiliate payout api normalizes Asaas provider fields and sends payout mutations', async () => {
    const originalFetch = globalThis.fetch;
    const requests = [];
    globalThis.fetch = (async (input, init) => {
        requests.push({ url: String(input), init });
    ...
    try {
        const payouts = await (0, admin_1.listAdminAffiliatePayouts)('token-123', { affiliateProfileId: 'aff-7' });
        const created = await (0, admin_1.createAdminAffiliatePayout)('token-123', {
            affiliateProfileId: 7,
            commissionIds: [1, 2],
            notes: 'Fechamento abril',
        });
        const updated = await (0, admin_1.updateAdminAffiliatePayoutStatus)('token-123', 'payout-2', {
            status: 'paid',
            statusReason: 'PIX manual confirmado',
        });
        const refreshed = await (0, admin_1.refreshAdminAffiliatePayout)('token-123', 'payout-2');
        strict_1.default.equal(payouts.items[0]?.amount, '42.00');
        strict_1.default.equal(payouts.items[0]?.affiliateProfileId, 'aff-7');
        strict_1.default.equal(payouts.items[0]?.notes, 'Pago no fechamento');
        strict_1.default.equal(payouts.items[0]?.statusReason, 'PIX confirmado');
        strict_1.default.equal(payouts.items[0]?.provider, 'asaas');
        strict_1.default.equal(payouts.items[0]?.externalReference, 'affiliate-payout-payout-1');
        strict_1.default.equal(payouts.items[0]?.providerTransactionId, 'tr_123');
        strict_1.default.equal(payouts.items[0]?.providerStatus, 'DONE');
        strict_1.default.equal(payouts.items[0]?.providerSyncedAt, '2026-04-20T12:01:00.000Z');
        strict_1.default.deepEqual(payouts.items[0]?.pixKey, { type: 'email', key: 'afiliado@example.com' });
        strict_1.default.equal(payouts.items[0]?.requestedAt, '2026-04-20T09:00:00.000Z');
        strict_1.default.equal(payouts.items[0]?.paidAt, '2026-04-20T12:00:00.000Z');
        strict_1.default.equal(payouts.items[0]?.processedAt, '2026-04-20T12:00:00.000Z');
        strict_1.default.equal(created.amount, '15.00');
        strict_1.default.equal(created.status, 'requested');
        strict_1.default.equal(created.commissionCount, 2);
        strict_1.default.deepEqual(created.commissionIds, ['1', '2']);
        strict_1.default.equal(updated.status, 'paid');
        strict_1.default.equal(updated.statusReason, 'PIX manual confirmado');
        strict_1.default.equal(updated.paidAt, '2026-04-20T14:00:00.000Z');
        strict_1.default.equal(refreshed.status, 'processing');
        strict_1.default.equal(refreshed.providerStatus, 'PENDING');
    }
    finally {
        globalThis.fetch = originalFetch;
    }
    ...
    strict_1.default.equal(requests[2].init?.body, JSON.stringify({
        status: 'paid',
        statusReason: 'PIX manual confirmado',
    }));
});
